...
function Get-FileSimilarity {
    ...
    try {
        $lines1 = Get-Content -Path $File1 -ErrorAction Stop
        $lines2 = Get-Content -Path $File2 -ErrorAction Stop
        ...
        # If there's an error reading the file, return 0 similarity
        Write-Host "Error comparing files: $File1 and $File2" -ForegroundColor Red
...
$files = Get-ChildItem -Path ./src -Recurse -File -Include "*.ts", "*.tsx", "*.js", "*.jsx" -Exclude "*.d.ts" | 
         Where-Object { $_.FullName -notlike "*\[*\]*" } # Filter out files with square brackets in the name
