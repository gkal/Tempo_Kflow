-- Create offer_history table to track all changes to offers
...
-- Create index for faster lookups by offer_id
...
-- Policy for insert (any authenticated user can insert)
CREATE POLICY insert_offer_history ON offer_history
  FOR INSERT TO authenticated
  WITH CHECK (true);

-- Policy for select (any authenticated user can view)
CREATE POLICY select_offer_history ON offer_history
  FOR SELECT TO authenticated
  USING (true);

-- Create a function to automatically record offer changes
...
-- Create a trigger to call the function on offer updates
