...
// Create a type for raw database entry to correctly handle the database responses
interface OfferHistoryDbEntry {
  id: string;
  offer_id: string;
  previous_status: string | null;
  new_status: string;
  ...
  const fetchOfferHistory = async () => {
    try {
      ...
      if (data && data.length > 0) {
        // Cast data to the proper database entry type
        const historyEntries = data as OfferHistoryDbEntry[];
        ...
        if (userIds.size > 0) {
          ...
          if (userError) {
            ...
            const userMap: Record<string, { id: string; fullname: string }> = {};
            userData.forEach(user => {
              userMap[user.id] = user;
            });

            // Enhance history entries with user information
            const enhancedData: HistoryEntry[] = historyEntries.map(entry => ({
              ...entry,
        ...
        // If we didn't get userData or there were no userIds, just set the raw data as history
        setHistory(historyEntries as HistoryEntry[]);
      } else {
        setHistory([]);
      }
    } catch (err) {
        ...
        {history.map((entry, index) => (
          ...
          >
            ...
            {entry.previous_status !== entry.new_status && (
              <div className="mb-3">
                ...
                <div className="flex items-center">
                  ...
                  <ArrowRight className="h-4 w-4 mx-2 text-[#84a98c]/60" />
                  <span className={`text-sm font-medium ${getStatusClass(entry.new_status)}`}>
            ...
            {entry.previous_assigned_to !== entry.new_assigned_to && (
              <div className="mb-3">
                <div className="text-xs text-[#84a98c] mb-1">Ανατέθηκε σε</div>
                <div className="flex items-center">
                  <span className="text-sm text-[#a8c5b5]">
                    {entry.previous_assigned_user?.fullname || "Μη ανατεθειμένο"}
                  </span>
                  <ArrowRight className="h-4 w-4 mx-2 text-[#84a98c]/60" />
                  <span className="text-sm font-medium text-[#cad2c5]">
                    {entry.new_assigned_user?.fullname || "Μη ανατεθειμένο"}
                  </span>
                </div>
              </div>
            )}
            ...
            {entry.previous_result !== entry.new_result && (
              <div className="mb-3">
                ...
                <div className="flex items-center">
                  ...
                  <ArrowRight className="h-4 w-4 mx-2 text-[#84a98c]/60" />
                  <span className={`text-sm font-medium ${getResultClass(entry.new_result)}`}>
            ...
            {entry.previous_amount !== entry.new_amount && (
              <div className="mb-3">
                ...
                <div className="flex items-center">
                  <span className="text-sm text-[#a8c5b5]">
                    {entry.previous_amount ? `${entry.previous_amount}€` : "—"}
                  </span>
                  <ArrowRight className="h-4 w-4 mx-2 text-[#84a98c]/60" />
                  <span className="text-sm font-medium text-[#cad2c5]">
                    {entry.new_amount ? `${entry.new_amount}€` : "—"}
                  </span>
                </div>
              </div>
            )}
            ...
            {entry.previous_requirements !== entry.new_requirements && (
              <div className="mb-3">
                ...
                {entry.previous_requirements && (
                  <div className="mb-2">
                    <div className="text-xs text-[#a8c5b5]/60 italic mb-1">Προηγούμενο:</div>
                    <div className="text-sm text-[#a8c5b5] bg-[#1c2b33] p-2 rounded">
                      {entry.previous_requirements}
                    </div>
                  </div>
                )}
                {entry.new_requirements && (
                  <div>
                    <div className="text-xs text-[#a8c5b5]/60 italic mb-1">Νέο:</div>
                    <div className="text-sm text-[#cad2c5] bg-[#1c2b33] p-2 rounded">
                      {entry.new_requirements}
                    </div>
                  </div>
                )}
            ...
            {entry.notes && (
              <div>
                ...
                <div className="text-sm text-[#cad2c5] italic">
                  {entry.notes}
                </div>
              </div>
            )}
